<template>
  <div class="page-shell ai-prompts-page">
    <AppSectionCard title="Prompt 模板" description="管理系统提示词模板、版本历史与启停状态。">
      <template #actions>
      ...
      <AppFilterBar>
        <el-input v-model="filters.q" clearable placeholder="搜索模板编码、名称或场景" @clear="handleSearch" @keyup.enter="handleSearch" />
    ...
    <section class="prompt-layout">
      <AppSectionCard title="模板列表" :unpadded="true">
        ...
        <el-result v-else-if="templateError" class="page-state" icon="warning" title="模板加载失败" :sub-title="templateError">
          <template #extra>
        ...
        <template v-else>
          ...
          <div class="table-footer">
            ...
            <CompactPager variant="project" :page-num="templatePageNum" :page-size="templatePageSize" :total="templateTotal" @change="handleTemplatePageChange" />
          </div>
        </template>
      </AppSectionCard>

      <AppSectionCard :title="selectedTemplateTitle" :description="selectedTemplateCode ? `模板编码：${selectedTemplateCode}` : '从左侧选择模板后查看版本详情。'" :unpadded="true">
