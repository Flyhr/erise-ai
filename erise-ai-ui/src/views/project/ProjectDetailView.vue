<template>
  <div v-if="project" class="page-shell">
    <AppPageHeader :title="project.name" :subtitle="project.description || '在这里统一查看项目资料总览，并快速进入文件或文档。'" show-back
    ...
    <AppSectionCard title="项目概览" :unpadded="true">
      ...
      <template #footer>
        <AppEmptyState v-if="!assets.length" title="项目内还没有资料" description="上传文件或创建文档后，这里会统一展示最新资料。" />
      </template>
    </AppSectionCard>
  </div>
</template>
...
import { formatDateTime, formatFileSize, isOfficeEditableFile, normalizeFileTypeLabel, projectStatusLabel, projectStatusTone } from '@/utils/formatters'
...
const load = async () => {
  const [projectDetail, assetPage] = await Promise.all([
    ...
    getKnowledgeAssets({ projectId, pageNum: 1, pageSize: 10 }),
  ])
